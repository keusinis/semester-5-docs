...
\begin{itemize}
    \item Learnability: 95\% of foreign workers will be able to perform bot basic and unusual PoS flows (order taking, different payment methods, refunds, check splitting, etc.) in less than 1 hour of training, if the user interface and item menu supports their native language.
...
\paragraph{Problems and Opportunities:} The problem arises when clients requests an invoice for their order in a non-physical format, which is only supported by one other provider, which does not fit the needs of the high-end restaurant. The ability to send invoices through e-mail in the PoS application is a great option when the customer is short on time, and generally is more convenient as a printed invoice. An additional opportunity is to have support for order-splitting at any given time during the orders lifespan, as customers may request to pay and leave separately.
...
\paragraph{\small Activities:} ordering food - very simple activity. High time pressure because it needs to be in time with the customer's lunch break. The task is well defined, has clear steps and occurs frequently.
\paragraph{\small Context:} restaurant environment, moderately noisy, many people due to lunch time. Individual activity, dependent on staff(organization).
\paragraph{\small Technologies:} no technologies used, but is affected by systems that influence the efficiency of food delivery process
